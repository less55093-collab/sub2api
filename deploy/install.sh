#!/bin/bash
#
# FluxRouter Installation Script
# FluxRouter 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/Wei-Shaw/fluxrouter/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="Wei-Shaw/fluxrouter"
INSTALL_DIR="/opt/fluxrouter"
SERVICE_NAME="fluxrouter"
SERVICE_USER="fluxrouter"
CONFIG_DIR="/etc/fluxrouter"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="FluxRouter 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 FluxRouter 服务："
    ...
    ["not_installed"]="FluxRouter 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 FluxRouter。"
    ...
    ["uninstall_complete"]="FluxRouter 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 FluxRouter"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 FluxRouter"
    ...
    ["server_config_desc"]="配置 FluxRouter 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="FluxRouter Installation Script"
    ...
    # Completion
    ["install_complete"]="FluxRouter installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start FluxRouter service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading FluxRouter..."
    ...
    ["not_installed"]="FluxRouter is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove FluxRouter from your system."
    ...
    ["uninstall_complete"]="FluxRouter has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install FluxRouter"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove FluxRouter"
    ...
    ["server_config_desc"]="Configure FluxRouter server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/fluxrouter" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/fluxrouter" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="fluxrouter_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/fluxrouter" "$INSTALL_DIR/fluxrouter"
    chmod +x "$INSTALL_DIR/fluxrouter"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/fluxrouter"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/fluxrouter.service << EOF
[Unit]
Description=FluxRouter - AI API Gateway Platform
Documentation=https://github.com/Wei-Shaw/fluxrouter
...
User=fluxrouter
Group=fluxrouter
WorkingDirectory=/opt/fluxrouter
ExecStart=/opt/fluxrouter/fluxrouter
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=fluxrouter
...
start_service() {
    ...
    if systemctl start fluxrouter; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u fluxrouter -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable fluxrouter 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status fluxrouter"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u fluxrouter -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart fluxrouter"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop fluxrouter"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if FluxRouter is installed
    if [ ! -f "$INSTALL_DIR/fluxrouter" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/fluxrouter" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet fluxrouter; then
        print_info "$(msg 'stopping_service')"
        systemctl stop fluxrouter
    fi

    # Backup current binary
    cp "$INSTALL_DIR/fluxrouter" "$INSTALL_DIR/fluxrouter.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/fluxrouter.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/fluxrouter"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start fluxrouter

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: FluxRouter must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/fluxrouter" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="fluxrouter.backup.${current_version}"
        else
            backup_name="fluxrouter.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/fluxrouter" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop fluxrouter 2>/dev/null || true
    systemctl disable fluxrouter 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/fluxrouter.service
    systemctl daemon-reload
