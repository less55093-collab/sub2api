# FluxRouter Deployment Files

This directory contains files for deploying FluxRouter on Linux servers.
...
| `fluxrouter.service` | Systemd service unit file |
| `fluxrouter-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/fluxrouter/main/deploy/docker-deploy.sh | bash

# Or download first, then run
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/fluxrouter/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
# View logs
docker compose -f docker-compose.local.yml logs -f fluxrouter

# If admin password was auto-generated, find it in logs:
docker compose -f docker-compose.local.yml logs fluxrouter | grep "admin password"
...
```bash
# Clone repository
git clone https://github.com/Wei-Shaw/fluxrouter.git
cd fluxrouter/deploy
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs fluxrouter | grep "admin password"
   ```
...
- 主进程固定探测 `/tmp/fluxrouter-datamanagement.sock`
...
# Restart FluxRouter only
docker compose -f docker-compose.local.yml restart fluxrouter
...
# View logs
docker compose logs -f fluxrouter

# Restart FluxRouter only
docker compose restart fluxrouter
...
| `ADMIN_EMAIL` | No | `admin@fluxrouter.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf fluxrouter-complete.tar.gz deployment/

# Transfer to new server
scp fluxrouter-complete.tar.gz user@new-server:/path/to/destination/

# On new server: Extract and start
tar xzf fluxrouter-complete.tar.gz
cd deployment/
docker compose -f docker-compose.local.yml up -d
```
...
FluxRouter supports three methods to connect to Gemini:
...
```bash
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/fluxrouter/main/deploy/install.sh | sudo bash
```

### Manual Installation

1. Download the latest release from [GitHub Releases](https://github.com/Wei-Shaw/fluxrouter/releases)
2. Extract and copy the binary to `/opt/fluxrouter/`
3. Copy `fluxrouter.service` to `/etc/systemd/system/`
4. Run:
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl enable fluxrouter
   sudo systemctl start fluxrouter
   ```
...
# Stop the service
sudo systemctl stop fluxrouter

# Restart the service
sudo systemctl restart fluxrouter

# Check status
sudo systemctl status fluxrouter

# View logs
sudo journalctl -u fluxrouter -f
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit fluxrouter
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/fluxrouter.service
   ```
...
The main config file is at `/etc/fluxrouter/config.yaml` (created by Setup Wizard).
...
```
/opt/fluxrouter/
├── fluxrouter              # Main binary
├── fluxrouter.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 fluxrouter
...
# View detailed logs
docker compose logs --tail=100 fluxrouter
...
# View recent logs
sudo journalctl -u fluxrouter -n 50

# Check config file
sudo cat /etc/fluxrouter/config.yaml
...
FluxRouter supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.fluxrouter.org](https://tls.fluxrouter.org/)** to get TLS fingerprint information for different devices and browsers.
