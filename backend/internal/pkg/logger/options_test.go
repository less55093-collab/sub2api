...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/fluxrouter-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/fluxrouter-data", "logs", "fluxrouter.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "fluxrouter.log")
