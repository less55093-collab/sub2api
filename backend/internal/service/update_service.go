...
const (
	...
	githubRepo     = "Wei-Shaw/fluxrouter"
...
func (s *UpdateService) PerformUpdate(ctx context.Context) error {
	...
	tempDir, err := os.MkdirTemp(exeDir, ".fluxrouter-update-*")
	if err != nil {
	...
	newBinaryPath := filepath.Join(tempDir, "fluxrouter")
...
func (s *UpdateService) extractBinary(archivePath, destPath string) error {
	...
	if strings.Contains(archivePath, ".tar") {
		...
		for {
			...
			if baseName == "fluxrouter" || baseName == "fluxrouter.exe" {
