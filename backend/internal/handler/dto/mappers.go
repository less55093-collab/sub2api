...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	if len(out.GroupIDs) == 0 && k.GroupID != nil {
		out.GroupIDs = []int64{*k.GroupID}
	}
	if len(k.Groups) > 0 {
		out.Groups = make([]Group, 0, len(k.Groups))
		for i := range k.Groups {
			g := k.Groups[i]
			if dtoGroup := GroupFromServiceShallow(&g); dtoGroup != nil {
				out.Groups = append(out.Groups, *dtoGroup)
			}
		}
	} else if k.Group != nil {
		out.Groups = []Group{*GroupFromServiceShallow(k.Group)}
	}
