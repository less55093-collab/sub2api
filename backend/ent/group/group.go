...
const (
	...
	// EdgeAPIKeysMulti holds the string denoting the api_keys_multi edge name in mutations.
	EdgeAPIKeysMulti = "api_keys_multi"
	...
	// APIKeysMultiTable is the table that holds the api_keys_multi relation/edge. The primary key declared below.
	APIKeysMultiTable = "api_key_groups"
	// APIKeysMultiInverseTable is the table name for the APIKey entity.
	// It exists in this package in order to avoid circular dependency with the "apikey" package.
	APIKeysMultiInverseTable = "api_keys"
...
var (
	// APIKeysMultiPrimaryKey and APIKeysMultiColumn2 are the table columns denoting the
	// primary key for the api_keys_multi relation (M2M).
	APIKeysMultiPrimaryKey = []string{"api_key_id", "group_id"}
...
// ByAPIKeysMultiCount orders the results by api_keys_multi count.
func ByAPIKeysMultiCount(opts ...sql.OrderTermOption) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborsCount(s, newAPIKeysMultiStep(), opts...)
	}
}

// ByAPIKeysMulti orders the results by api_keys_multi terms.
func ByAPIKeysMulti(term sql.OrderTerm, terms ...sql.OrderTerm) OrderOption {
	return func(s *sql.Selector) {
		sqlgraph.OrderByNeighborTerms(s, newAPIKeysMultiStep(), append([]sql.OrderTerm{term}, terms...)...)
	}
}
...
func newAPIKeysMultiStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		sqlgraph.From(Table, FieldID),
		sqlgraph.To(APIKeysMultiInverseTable, FieldID),
		sqlgraph.Edge(sqlgraph.M2M, true, APIKeysMultiTable, APIKeysMultiPrimaryKey...),
	)
}
