...
func (_c *GroupCreate) AddAPIKeysMultiIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddAPIKeysMultiIDs(ids...)
	return _c
}

// AddAPIKeysMulti adds the "api_keys_multi" edges to the APIKey entity.
func (_c *GroupCreate) AddAPIKeysMulti(v ...*APIKey) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddAPIKeysMultiIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.APIKeysMultiIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
